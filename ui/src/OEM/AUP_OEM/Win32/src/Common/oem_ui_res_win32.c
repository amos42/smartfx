...
static atBOOL	OEM_UI_InitRes(atLONG lSysInst, atLONG lInitData1, atLONG lInitData2)
{
	//atBOOL r = atFALSE;
	//atRESFUNCS lstFuncs;

	atUIAPI_memset( &g_lstFuncs, 0, sizeof(atRESFUNCS) );
	g_lstFuncs.lpData = atNULL;
	g_lstFuncs.fnAllocator = atUIAPI_AllocMem;
	g_lstFuncs.fnDeallocator = (atLPCALLBACKFREE)atUIAPI_FreeMem;
	g_lstFuncs.fnMemSet = atUIAPI_memset;
...
static void	OEM_UI_FinalRes(void)
{
}

static OEM_RESSRC	OEM_UI_CreateResSource( atLPTSTR szIDStr, int type, void *ptr, long size, atLPCALLBACKALLOC fnAllocator, atLPCALLBACKFREE fnDeallocator )
{
	atHRESOURCE hRes;
...
static atBOOL	OEM_UI_RemoveResSource( OEM_RESSRC hOemResSrc )
{
	atRES_DestroyResource( hOemResSrc );
	return atTRUE;
}

static atBOOL OEM_UI_GetResSourceInfo(OEM_RESSRC hOemResSrc, atUIAPI_RESSRC_INFO *lpResSrcInfo)
{
...
static atBOOL	OEM_UI_SetResAllocator( OEM_RESSRC hOemResSrc, atLPCALLBACKALLOC fnAllocator, atLPCALLBACKFREE fnDeallocator )
{
/*
...
static atBOOL	OEM_UI_GetResAllocator( OEM_RESSRC hOemResSrc, atLPCALLBACKALLOC *lpfnAllocator, atLPCALLBACKFREE *lpfnDeallocator )
{
...
static atBOOL OEM_UI_IsExistRes( OEM_RESSRC hOemResSrc, atLPTSTR pFileName )
{
...
static atLONG OEM_UI_GetResInfo(OEM_RESSRC hOemResSrc, atLPTSTR szFileName, atUIAPI_RES_INFO *lpResInfo)
{
	atANSI_CHAR filename[128];
	int idx;
	atRES_FILE_INFO fi;
...
static OEM_RES	OEM_UI_LoadRes( OEM_RESSRC hOemResSrc, atLPTSTR szFileName )
{
	atRESOURCE *lpRes;
	atANSI_CHAR filename[128];
	if (atUIAPI_ToANSI( filename, szFileName, -1 ) == 0) return 0;
...
static atBOOL OEM_UI_GetResData( OEM_RES hOemRes, atLPVOID *lppPtr, atLONG *lpSize )
{
...
static atVOID OEM_UI_FreeRes( OEM_RES hOemRes )
{
...
void init_uiapi_res_oem() 
{
	OEM_UI_RES_T oem_funcs = {
		OEM_UI_InitRes,
		OEM_UI_FinalRes,

		OEM_UI_CreateResSource,
		OEM_UI_RemoveResSource,
		OEM_UI_GetResSourceInfo,

		OEM_UI_SetResAllocator,
		OEM_UI_GetResAllocator,
