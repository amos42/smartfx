...
//    FT_Library ftLibrary;
	...
	if (nDataType == 0) {
		...
		atHRESSRC ressrc = atUIAPI_GetResSource((atLPTSTR)(atINTPTR)nDataType);
		if (ressrc != atNULL) {
			...
			if (ressrcinfo.type == 3 && resinfo.nCompressMethod != 1) {
				...
				if (fontres != atNULL) atUIAPI_GetResData(fontres, &g_font_mem, &g_font_size);
			}
		}
	}
...
//	lf.lfHeight = nSize;
	...
	return (atGRPX_OEM_FONT)hFontDC;
}
...
static atINT		OEM_GetStringWidth( OEM_UI_FONT hFont, atLPTSTR str, atINT len )
{	
	return atGRPX_GetStringWidth( (atGRPX_FONT*)hFont, str, len );
}

static atINT		OEM_GetFontHeight( OEM_UI_FONT hFont )
{
	return atGRPX_GetFontHeight( (atGRPX_FONT*)hFont );
}

static atINT		OEM_GetGlyph( OEM_UI_FONT hFont )
{
	return atGRPX_GetFontHeight( (atGRPX_FONT*)hFont );
}
