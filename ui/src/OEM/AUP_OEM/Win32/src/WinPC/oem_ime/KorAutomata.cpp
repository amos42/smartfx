...
void CKoreanAutomata::Flush ()
{
	if(State())													//조합중인 코드가 있으면 조합중인 코드도 출력한다.
	{
		unsigned short RealCode = GetConvertedUnionCode();		
		char buff[3] = {RealCode & 0x00ff,RealCode>>8, 0};					//출력을 위해 char형 변수에 저장한다.
		char cpPrev[STRINGBUFFER_SIZE];

		const char* cpHead = GetStringBuffer();
		int iLen = strlen(cpHead);

		strncpy (cpPrev, cpHead, m_StrIndex);
		cpPrev[m_StrIndex]   = buff[0];
		cpPrev[m_StrIndex+1] = buff[1];
		cpPrev[m_StrIndex+2] = '\0';

		strncpy (cpPrev+m_StrIndex+2, cpHead+m_StrIndex, iLen-m_StrIndex);
		cpPrev[iLen+2] = '\0';

		strcpy (m_DestStrBuffer, cpPrev);

		// index 이동시키기.
		m_StrIndex += 2;

		// Set InStackIndex by flush
		m_InStackIndex = 0;
	}
}
...
{	
	/*-------------------------------------------------------------------- 
	인수	: key = 입력된 키 코드 
	반환값	: 한 글자의 조합이 끝나면 1을, 계속 조합중이면 0을, 완성된 
			: 글자의 코드는 입력 스택의 가장 마지막에서 구할 수 있다.  
	동작	:  
	--------------------------------------------------------------------*/ 	
...
int CKoreanAutomata::GetUnicodeBase_StringCount ()
{
	unsigned char cpBuff[STRINGBUFFER_SIZE];
	strcpy ((char*)cpBuff, m_DestStrBuffer);

	if(State()) //조합중인 코드가 있으면 조합중인 코드도 출력한다.
	{
		unsigned short nRealCode = GetConvertedUnionCode();
		char cpIng[3] = {nRealCode & 0x00ff, nRealCode>>8, 0};	//출력을 위해 char형 변수에 저장한다.
		strcat ((char*)cpBuff, cpIng);
	}

	unsigned char* cpStr = cpBuff;
	int iLen = strlen((char*)cpStr);

	int nCount = 0;
	int iHanStart = 0;

	for (int nn=0; nn<iLen; nn++)
	{
		if (iHanStart)
		{
			nCount++;
...
int CKoreanAutomata::SetStringBuffer(char* _cpBuffer )
{
	if (_cpBuffer == NULL)
		return 0;
	
	int iLen = strlen(_cpBuffer) ;
	if (iLen >= STRINGBUFFER_SIZE)
		return 0;
	
	strcpy (m_DestStrBuffer, _cpBuffer);
	m_StrIndex = iLen;
	...
	if(RealKey == ConvertedKey)										// 한글입력 이외의 키를 입력받았다.
	{
		if(RealKey == 8)											//백스페이스 일 경우
		{
			if(BackSpace() == false)								//더 지울 조합이 없다. (오토마타가 HS_START 상태가 됨)
			{
				if(m_StrIndex)
				{
					if(IsHangul(m_DestStrBuffer, m_StrIndex-1))
