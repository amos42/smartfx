...
/*
		case WM_ACTIVATE:
			// 최상위 레벨 윈도우가 WM_ACTIVATE 메시지를 받았을때 그에대한 응답으로 호출하는 함수.
			memset( &g_sai, 0, sizeof(SHACTIVATEINFO) );
			g_sai.cbSize = sizeof(SHACTIVATEINFO);
			SHHandleWMActivate(hWnd, wParam, lParam, &g_sai, FALSE);
			break;

		case WM_SETTINGCHANGE:
			// 이 윈도우에 포커싱 오기전의 SIP 설정을 복원한다.
...
static atBOOL OEM_UI_InitOIC( atLONG lSysInst, atLONG lInitData1, atLONG lInitData2 )
{
...
static void OEM_UI_FinalOIC(void)
{
...
static atBOOL OEM_UI_IsSupportedOIC(void)
{
	return atTRUE;
}
...
static int _xxx( long param )
{
	...
	switch( params->funcno ){
		case 1 :id = find_free_id();
				...
				hEditBox = CreateWindow( __TEXT("Edit"), __TEXT(""), WS_CHILD, 0, 0, 50, 18, g_UI_hWindow, (HMENU)id, g_UI_hInstance, NULL );
...
#ifdef _WIN32_WCE
				SHSipPreference( (HWND)params->hOemOIC, SIP_DOWN );
#endif				
...
#ifdef _WIN32_WCE
    			SHSipPreference( (HWND)params->hOemOIC, SIP_UP );
//    			SHSipPreference( (HWND)g_UI_hWindow, SIP_UP );
//				SipShowIM(SIPF_ON);
#endif
				break;
...
static void OEM_UI_SetOICSize( OEM_UI_OIC hOemOIC, int width, int height );
static void OEM_UI_MoveOIC( OEM_UI_OIC hOemOIC, int x, int y );

static OEM_UI_OIC OEM_UI_CreateOIC( int x, int y, int width, int height, atUI_FUNC_OIC_NOTIFY *fnFunc, void *param )
{
...
static void OEM_UI_DestroyOIC( OEM_UI_OIC hOemOIC )
{
	invoke_params params;
...
static void OEM_UI_SetOICFont( OEM_UI_OIC hOemOIC, int font_size )
{
	invoke_params params;
	params.hOemOIC = hOemOIC;
	params.funcno = 3;
...
static void OEM_UI_GetOICPos( OEM_UI_OIC hOemOIC, int *x, int *y )
{
	invoke_params params;
	params.hOemOIC = hOemOIC;
	params.funcno = 4;
...
static void OEM_UI_GetOICSize( OEM_UI_OIC hOemOIC, int *width, int *height )
{
	invoke_params params;
	params.hOemOIC = hOemOIC;
	params.funcno = 5;
...
static void OEM_UI_ShowAndSetFocusOIC( OEM_UI_OIC hOemOIC )
{
	invoke_params params;
	params.hOemOIC = hOemOIC;
	params.funcno = 8;
...
static void OEM_UI_HideAndLostFocusOIC( OEM_UI_OIC hOemOIC )
{
	invoke_params params;
	params.hOemOIC = hOemOIC;
	params.funcno = 9;
...
static void OEM_UI_RefreshOIC( OEM_UI_OIC hOemOIC )
{
	invoke_params params;
	params.hOemOIC = hOemOIC;
	params.funcno = 10;
...
static void OEM_UI_SetOICText( OEM_UI_OIC hOemOIC, atLPCTSTR szText, atINT nLen )
{
	invoke_params params;
	params.hOemOIC = hOemOIC;
	params.funcno = 11;
...
static void OEM_UI_GetOICText( OEM_UI_OIC hOemOIC, atLPTSTR szText, atINT nLen )
{
	invoke_params params;
	params.hOemOIC = hOemOIC;
	params.funcno = 12;
...
void init_uiapi_oic_oem() 
{
	OEM_UI_OIC_T oem_funcs = {
		OEM_UI_InitOIC,
		OEM_UI_FinalOIC,
		OEM_UI_IsSupportedOIC,
		...
		OEM_UI_SetOICSize,
		OEM_UI_MoveOIC,
		OEM_UI_ShowAndSetFocusOIC,
		OEM_UI_HideAndLostFocusOIC,
		OEM_UI_RefreshOIC,
		OEM_UI_SetOICText,
		OEM_UI_GetOICText
	};

	OEM_UI_OIC_SetOemFn( &oem_funcs );
}
