...
atBOOL RegistEditBoxExWndClass(void)
{
	...
	return atAPP_RegistWinClass( atAPPMNG_GetCurApp(), EXTPKG_CUSTWIN_EDITBOX, &wc );
}


atVOID UnregistEditBoxExWndClass(void)
{
}
...
static void	EditBoxWndOnInit(atHWINDOW hWnd, struct _CreateParam *param)
{
	...
	static atEDITBOX_EXT_INPUTMODE lstInput[3] = {
	...
	if( param->bUseOIC ){
		lpData->hEditBox = ExtPkg_CreateOICEditBox( hWnd, 103, 10, 60, 50, 22, param->text, lstInput, 3, 0 );
	} else {
		lpData->hEditBox = ExtPkg_CreateEditBox( hWnd, 103, 10, 60, 50, 22, param->text, lstInput, 3, 0 );
	}
	atCONTROL_SetAnchor( lpData->hEditBox, atANCHOR_IGNORE, atANCHOR_IGNORE, 10, 10 );

	lpData->hBtnOK = ExtPkg_CreateButton( hWnd, 104, 20, 100, 60, 18, _AT("OK") );
	lpData->hBtnCancel = ExtPkg_CreateButton( hWnd, 105, 110, 100, 60, 18, _AT("Cancel") );
