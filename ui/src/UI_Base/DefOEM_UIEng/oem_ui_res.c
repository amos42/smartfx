...
{
	atRESMNGFUNCS lstFuncs;
	
	lstFuncs.fnAllocMem = atUIAPI_AllocMem;
	lstFuncs.fnFreeMem = (atLPCALLBACKFREE)atUIAPI_FreeMem;
	lstFuncs.fnStrCopy = atUIAPI_strncpy;
	lstFuncs.fnLoadFile = atUIAPI_LoadFile;

	atBOOL r = atRESMNG_InitializeResourceMng(&lstFuncs);
...
OEM_RESSRC	OEM_UI_CreateResSource( atLPTSTR szIDStr, int type, void *ptr, long size, atLPCALLBACKALLOC fnAllocator, atLPCALLBACKFREE fnDeallocator )
{
	...
	hRes = atRES_CreateResourceEx( atNULL, ptr, size, type, fnAllocator, fnDeallocator );
