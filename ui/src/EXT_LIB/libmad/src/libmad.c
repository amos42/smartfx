...
void* mad_init()
{
	dec_struct* dec = (dec_struct*)SndHeapAlloc(sizeof(dec_struct));
	if (!dec) return NULL;
			   ...
			   int outmemsize, int* read, int *done, int resolution, int halfsamplerate)
{
	...
	*read = 0;
	...
	if (dec->left) {
		...
		pack_pcm((unsigned char*)outmemory, size, &dec->synth.pcm.samples[0][dec->synth.pcm.length - dec->left],
	...
	mad_stream_buffer(&dec->stream, (unsigned char*)inmemory, inmemsize);
	...
	if (outmemsize < dec->synth.pcm.length * resolution * dec->synth.pcm.channels / 8) {
		...
		pack_pcm((unsigned char*) outmemory + *done, size, dec->synth.pcm.samples[0],
			(dec->synth.pcm.channels == 1) ? NULL : dec->synth.pcm.samples[1], resolution, &dec->stats);
		*done += size * resolution * dec->synth.pcm.channels / 8;
		*read += dec->stream.next_frame - (unsigned char*)inmemory;
		dec->left = dec->synth.pcm.length - size;
		return MAD_NEED_MORE_OUTPUT;
	}

	pack_pcm((unsigned char*)outmemory + *done, dec->synth.pcm.length, dec->synth.pcm.samples[0],
		(dec->synth.pcm.channels == 1) ? NULL : dec->synth.pcm.samples[1], resolution, &dec->stats);
