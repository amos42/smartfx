...
/********************
   문자열을 찍는다.
*********************/
void __atTGRPX_DrawString0( atTGRPX_FONT *lpFont, atLPBYTE lpDes, int nBPP, int widthbytes, atRECT *clip, atINT nX, atINT nY, atLPTSTR szStrData, atINT nLen, atINT nLineGap,
						   		atLONG ExParam1,
...
//	int descent;
	...
	atTGRPX_GetFontSize( lpFont, &ascent, atNULL );
...
//	descent = lpFont->nDescent;
    ...
    fontheight = atTGRPX_GetFontHeight( lpFont );
...
//	  if( (yy > clip->nEndY) ) break;
	  ...
	  if( ch == _ATC('\n') ){
		...
		int xx2, yy2;
		r = atTGRPX_GetGlyph( lpFont, atTGRPX_GetCharIndex(lpFont, ch), &glp );
		if( !r ) continue;
...
yy += (glp.rAdvY);
		  ...
		  atTGRPX_ReleaseGlyph( lpFont, &glp );
	      }
	}
}
