...
**********************************/
void atGRPX_FillRect( atGRPX_CANVAS *lpCanvas, atINT StartX, atINT StartY, atINT EndX, atINT EndY, atGRPX_COLOR Color )
{
    int i, j;
...
//    if( Color == TRANS_COLOR ) return;
    ...
    for( i = 0; i < height; i ++ ){
      for( j = 0; j < width; j ++ ){
        *p++ = pixel;
//atGRPX_DrawPixelTrans( lpCanvas, StartX+j, StartY+i, Color, 128);
      }
