...
static void _LOCAL_FUNC_(_memset)(void *d, atCHAR ch, int len)
{
	atCHAR *dptr = (atCHAR *)d;
	while( len-- ) *dptr++ = ch;
}
...
atINT _GLOBAL_FUNC_(_astr_vsnscanf)( _TCHAR *buf, int buflen, const _TCHAR *fmt, va_list args )
{
	atFLOAT *f;
	...
	for( fp = (_TCHAR *)fmt, tp = (_TCHAR *)fmt; *fp != _XT('\0'); fp++ ) {
		...
		// 버퍼 초기화
		_LOCAL_FUNC_(_memset)(szTemp, '\0', sizeof(_TCHAR) * _AT_BUFSIZ);
