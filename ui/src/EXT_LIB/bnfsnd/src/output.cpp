...
COutput::COutput()
{
	...
	m_dwTotalSamples = 0;
	m_fPaused = FALSE;
	m_fDoubleBuf = TRUE; // BlueBird ｸﾘﾃﾟｴﾂ ｹｮﾁｦｶｧｹｮｿ｡ FALSE->TRUEｷﾎ ｼ､. 
	m_hEventThread = NULL;
	m_hThread = NULL;
	m_dwThreadId = 0;
	m_pbSubBuf = NULL;
	...
	// Coverity Errors :: #801ｹｮﾁｦ. ﾃﾊｱ篳ｭ.
	memset(m_HdrClear, 0, sizeof(m_HdrClear));
	m_nFadeCurrent = 0;
	m_nFadeRate = 0;
	m_nSubBuf = 0;	
}
...
BOOL COutput::PrepareBuffer()
{
	...
	m_pHdr = (WAVEHDR*)SndHeapAlloc(sizeof(WAVEHDR)*m_cBuf);
	if (!m_pHdr)
		return FALSE;

	m_pbBuf = (BYTE*)SndHeapAlloc(m_cBuf * m_cbBuf);
	if (!m_pbBuf)
		return FALSE;
...
BOOL COutput::PrepareSubBuffer()
{
	...
	m_pbSubBuf = (BYTE*)SndHeapAlloc(cb * SUBBUF_COUNT);
