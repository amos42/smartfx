...
void CDelayEffect::Open(int nChannels, int nSamplingRate)
{
	...
	m_pBuf = (int*)SndHeapAlloc(sizeof(int)*m_cBuf);
	if(!m_pBuf) return;
...
{
	m_pBuf[0] = (int*)SndHeapAlloc(sizeof(int)*nSampleRate);
	m_pBuf[1] = (int*)SndHeapAlloc(sizeof(int)*nSampleRate);
...
{
	if (m_pBuf[0]) {
		SndHeapFree(m_pBuf[0]);
		m_pBuf[0] = NULL;
	}
	if (m_pBuf[1]) {
		SndHeapFree(m_pBuf[1]);
		m_pBuf[1] = NULL;
	}
	m_nSampleRate = 0;
}
