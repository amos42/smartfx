...
{
	atLPDISPLISTENER lpOldDispFunc = g_lpDispFunc;
	g_lpDispFunc = lpDispFunc;
	return lpOldDispFunc;
}
...
{
	//g_hDispProc = atNULL; // atUIAPI_CreateProcessor(_AT("DispProc"), 0, _dispproc, atNULL);
...
{
	if (g_lpDispFunc) {
		g_lpDispFunc(EVENT_DISP_SCREEN_LOCKED);
	}
	...
	if (g_lpDispFunc) {
		g_lpDispFunc(EVENT_DISP_SCREEN_UNLOCKED);
		g_lpDispFunc(EVENT_DISP_INVALID_ADDED);
	}
}
